resource "null_resource" "WebserverHTTPD_prueba" {
 count      = var.NumberOfNodes   
 depends_on = [oci_core_instance.Webserver,oci_core_instance.BastionServer]
 provisioner "remote-exec" {
        connection {
                type     = "ssh"
                user     = "opc"
		        host     = data.oci_core_vnic.Webserver_VNIC1[count.index].private_ip_address
